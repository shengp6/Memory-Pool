...
public class HashTable {
     ...
     */
    public HashTable(int size, MemoryPool mp) {
        this.size = size;
        numOfEntries = 0;
     ...
     */
    private void resize() {      
        HashTable temp = new HashTable(size * 2, mp);
        for (int i = 0; i < this.size; i++) {
            if (table[i] != null) {
                temp.insert(mp.getEntry(table[i]), table[i]);
            }
        }
        this.size = temp.size;
     ...
     * @return The handle of the string if exists, null if not
     */
    public int search(String str) {
        int pos = hashing(str, size);
        if (table[pos] != null 
                && str.equals(mp.getEntry(table[pos]))) {             
            return pos;            
        }
        for (int i = 0; i < size; i++) {
            int index = (pos + i * i) % size;
            if (table[index] != null 
                    && str.equals(mp.getEntry(
                            table[index]))) {                                  
                return index;              
            }
        }
